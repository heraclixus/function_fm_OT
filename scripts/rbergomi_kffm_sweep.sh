...
# 
# Goal: Improve k-FFM autocorrelation_mse (currently 2.3x worse than DDPM)
#       on rough volatility paths with H=0.1
# 
# Usage:
#   ./rbergomi_kffm_sweep.sh                  # Run all configs sequentially
#   ./rbergomi_kffm_sweep.sh --parallel 4     # Run 4 configs in parallel
#   ./rbergomi_kffm_sweep.sh --config sig_leadlag_order2  # Run single config
#   ./rbergomi_kffm_sweep.sh --dry-run        # Show what would be run
# =============================================================================
...
CONFIG_FILE="${PROJECT_ROOT}/configs/rbergomi_signature_sweep.yaml"
OUTPUT_DIR="${PROJECT_ROOT}/outputs/rbergomi_signature_sweep"
EPOCHS=100
N_PARALLEL=1
DRY_RUN=false
SINGLE_CONFIG=""
ALPHA="-0.4"  # Hurst parameter H = alpha + 0.5 = 0.1
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --output DIR     Output directory (default: outputs/rbergomi_signature_sweep)"
            echo "  --epochs N       Number of epochs (default: 100)"
            echo "  --alpha VAL      Roughness parameter (default: -0.4, H=0.1)"
...
CONFIGS=(
    # Lead-lag experiments (priority for autocorrelation)
    "sig_leadlag_order1"
    "sig_leadlag_order2"
    "sig_leadlag_order3"
    ...
    # Static kernel sigma sweep (smaller values for rough paths)
    "sig_sigma0.1"
    "sig_sigma0.5"
    "sig_sigma2.0"
...
echo "============================================================"
echo "Phase 1: Signature Kernel Hyperparameter Sweep for rBergomi"
echo "============================================================"
echo "Config file: $CONFIG_FILE"
echo "Output dir:  $OUTPUT_DIR"
echo "Alpha:       $ALPHA (H=$(echo "$ALPHA + 0.5" | bc))"
echo "Epochs:      $EPOCHS"
...
run_experiment() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        echo "  DRY RUN: python rBergomi_ot.py --config-file $CONFIG_FILE --config $config --seed $seed --epochs $EPOCHS --alpha $ALPHA --spath $OUTPUT_DIR"
        return 0
    fi
...
export CONFIG_FILE OUTPUT_DIR EPOCHS ALPHA LOG_DIR DRY_RUN SCRIPT_DIR
...
if [[ "$DRY_RUN" != "true" ]]; then
    ...
    echo "# rBergomi Signature Kernel Sweep Results" > "$SUMMARY_FILE"
    echo "# Generated: $(date)" >> "$SUMMARY_FILE"
    echo "# Alpha: $ALPHA (H=$(echo "$ALPHA + 0.5" | bc))" >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"
    
    for config in "${CONFIGS[@]}"; do
        echo "Config: $config" >> "$SUMMARY_FILE"
        for seed in "${SEEDS[@]}"; do
            metrics_file="${OUTPUT_DIR}/${config}/seed_${seed}/quality_metrics.json"
            if [[ -f "$metrics_file" ]]; then
                ...
                kurtosis=$(python -c "import json; d=json.load(open('$metrics_file')); print(f'{d.get(\"kurtosis_mse\", \"N/A\"):.2e}')" 2>/dev/null || echo "N/A")
                echo "  seed_$seed: autocorr=$autocorr, mean=$mean_mse, kurtosis=$kurtosis" >> "$SUMMARY_FILE"
            else
    ...
    echo "  python rBergomi_ot.py --load-only --spath $OUTPUT_DIR"
fi
