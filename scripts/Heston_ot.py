"""
Comprehensive OT-FFM Experiments on Heston Stochastic Volatility.

This script runs experiments with multiple OT configurations on Heston
...
Usage:
    python Heston_ot.py
    python Heston_ot.py --kappa 2.0  # Faster mean reversion
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description='Heston OT-FFM Experiments')
...
parser.add_argument('--spath', type=str, default=None, help='Output/load directory (default: auto-generated)')
...
n_x = args.n_steps
kappa = args.kappa
sigma_heston = args.sigma

print(f"Heston params: κ={kappa}, σ={sigma_heston}")

# Generate dataset
print("Generating Heston dataset...")
...
if args.spath:
    ...
    spath = Path(f'../outputs/Heston_ot_kappa{kappa}/')
...
def plot_samples_comparison(aggregated: Dict, ground_truth: torch.Tensor, save_path: Path):
    ...
    for idx, (config_name, data) in enumerate(aggregated.items()):
        ...
        ax.set_title(f'Ground Truth\n(Heston κ={kappa})', fontsize=11, fontweight='bold')
        ...
        plt.suptitle(f'Heston: Ground Truth vs {config_name}', fontsize=12, y=1.02)
        plt.tight_layout()
...
if __name__ == "__main__":
    print("="*60)
    print(f"OT-FFM Experiments on Heston (κ={kappa}, σ={sigma_heston})")
    ...
    if args.baselines_only:
        # Run only baselines, then load existing OT results
    ...
    # Filter out baselines for OT-specific plots
