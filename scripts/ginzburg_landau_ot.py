"""
Comprehensive OT-FFM Experiments on Stochastic Ginzburg-Landau.

This script runs experiments with multiple OT configurations on stochastic
Ginzburg-Landau equation solutions comparing:
...
Usage:
    python ginzburg_landau_ot.py
    python ginzburg_landau_ot.py --epochs 100 --n_seeds 5
"""
...
parser.add_argument('--dpath', type=str, default='../data/stochastic_ginzburg_landau.mat')
parser.add_argument('--spath', type=str, default='../outputs/ginzburg_landau_ot/')
parser.add_argument('--ntr', type=int, default=40000, help='Training samples')
...
data = load_ginzburg_landau(args.dpath, shuffle=True, mode='snapshot')
...
def plot_samples_comparison(aggregated: Dict, ground_truth: torch.Tensor, save_path: Path):
    ...
    for idx, (config_name, data) in enumerate(aggregated.items()):
        ...
        ax.set_title('Ground Truth\n(Stochastic Ginzburg-Landau)', fontsize=11, fontweight='bold')
        ...
        plt.suptitle(f'Ginzburg-Landau: Ground Truth vs {config_name}', fontsize=12, y=1.02)
        plt.tight_layout()
...
def create_summary_report(aggregated: Dict, save_path: Path):
    """Create summary JSON."""
    summary = {
        'dataset': 'Stochastic Ginzburg-Landau',
        'n_samples': ntr,
        'n_x': n_x,
        'epochs': epochs,
...
if __name__ == "__main__":
    ...
    print("="*60)
    print(f"OT-FFM Experiments on Stochastic Ginzburg-Landau")
