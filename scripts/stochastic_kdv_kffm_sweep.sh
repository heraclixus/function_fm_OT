#!/bin/bash
# ============================================================================
# Stochastic KdV k-FFM Hyperparameter Sweep Script
# ============================================================================
# Purpose: Improve k-FFM to beat FFM (independent) on mean_mse
# Current: FFM=0.000934, Best k-FFM=0.00137 (euclidean_sinkhorn_reg0.1)
# Target: k-FFM mean_mse < 0.00094
#
# Usage:
#   ./stochastic_kdv_kffm_sweep.sh                    # Run all configs sequentially
#   ./stochastic_kdv_kffm_sweep.sh --parallel 4       # Run with 4 parallel jobs
#   ./stochastic_kdv_kffm_sweep.sh --dry-run          # Show what would run
#   ./stochastic_kdv_kffm_sweep.sh --config euclidean_sinkhorn_reg0.05 --seed 1
# ============================================================================
...
CONFIG_FILE="${SCRIPT_DIR}/../configs/stochastic_kdv_sweep.yaml"
PYTHON_SCRIPT="${SCRIPT_DIR}/stochastic_kdv_ot.py"
OUTPUT_DIR="${SCRIPT_DIR}/../outputs/stochastic_kdv_ot"
SEEDS=(1 2 4)
EPOCHS=100
PARALLEL_JOBS=1
DRY_RUN=false
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}Stochastic KdV k-FFM Hyperparameter Sweep${NC}"
echo -e "${BLUE}============================================${NC}"
echo ""
...
if [[ "$DRY_RUN" != "true" ]]; then
    ...
    echo "  python stochastic_kdv_ot.py --load-only"
    echo ""
    
    # Quick summary of results
    echo -e "${BLUE}Quick Summary of mean_mse (lower is better):${NC}"
    echo "Target to beat: FFM (independent) = 0.000934"
    echo ""
    
    for config in $CONFIGS; do
        # Check if results exist
        if [[ -d "${OUTPUT_DIR}/${config}" ]]; then
            # Try to extract mean_mse from quality_metrics.json
            mean_mse=$(python -c "
import json
import numpy as np
values = []
for seed in [1, 2, 4]:
    try:
        with open('${OUTPUT_DIR}/${config}/seed_' + str(seed) + '/quality_metrics.json') as f:
            data = json.load(f)
            if 'mean_mse' in data and data['mean_mse'] is not None:
                values.append(data['mean_mse'])
    except: pass
if values:
    print(f'{np.mean(values):.6f}')
else:
    print('N/A')
" 2>/dev/null || echo "N/A")
            
            if [[ "$mean_mse" != "N/A" ]]; then
                # Compare to baseline
                better=$(python -c "print('✓' if $mean_mse < 0.000934 else '✗')" 2>/dev/null || echo "?")
                echo "  $config: $mean_mse $better"
            fi
        fi
    done
fi
