"""
Comprehensive OT-FFM Experiments on Stochastic Navier-Stokes.

This script runs experiments with multiple OT configurations on stochastic
Navier-Stokes equation solutions (2D spatial) comparing:
...
Using snapshot mode: each 2D spatial snapshot is treated as i.i.d. sample.

Usage:
    python stochastic_ns_ot.py
    python stochastic_ns_ot.py --epochs 50 --n_seeds 3
"""
...
parser.add_argument('--dpath', type=str, default='../data/stochastic_ns_64.mat')
...
def train_baseline_config(
    ...
    gt_flat = ground_truth[:1000].reshape(min(1000, ground_truth.shape[0]), -1)
    samples_flat = samples.reshape(n_gen_samples, -1)
...
def plot_samples_grid(aggregated: Dict, ground_truth: torch.Tensor, save_path: Path):
    ...
    for config_name, data in aggregated.items():
        ...
        plt.suptitle(f'Stochastic NS: {config_name}', fontsize=12)
        plt.tight_layout()
...
def create_summary_report(aggregated: Dict, save_path: Path):
    """Create summary JSON."""
    summary = {
        'dataset': 'Stochastic Navier-Stokes',
        'n_samples': ntr,
...
if __name__ == "__main__":
    ...
    print("="*60)
    print(f"OT-FFM Experiments on Stochastic Navier-Stokes")
