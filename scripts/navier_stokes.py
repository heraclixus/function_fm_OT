"""
FFM Experiments on Navier-Stokes Data.

This script runs FFM/DDPM/NCSN/GANO experiments on Navier-Stokes 
equation solutions (2D spatial).

Data shape: (batch_size, N, N, T) - typically (10, 64, 64, 15001)

Note: The data has many time steps. Use --subsample-time to reduce.

Usage:
    python navier_stokes.py --dpath ../data/ns.mat --spath ../outputs/ns --method FFM ...
"""
...
if __name__ == '__main__':
    ...
    # Data preprocessing
    parser.add_argument('--subsample-time', help='Subsample every n-th time step', type=int, default=5)
    ...
    # Training params
    parser.add_argument('--ntr', help='Number of training datapoints', type=int, default=20000)
    parser.add_argument('--nte', help='Number of testing datapoints', type=int, default=5000)
    ...
    print('##### Loading Data')
    subsample = getattr(args, 'subsample_time', 1)
    u = load_navier_stokes(args.dpath, shuffle=True, subsample_time=subsample)
    print(f'Loaded data shape: {u.shape}')
    
    # Ensure we have enough samples
    n_total = u.shape[0]
    if args.ntr + args.nte > n_total:
        print(f'Warning: Requested {args.ntr + args.nte} samples but only {n_total} available.')
        args.ntr = min(args.ntr, int(0.8 * n_total))
        args.nte = min(args.nte, n_total - args.ntr)
        print(f'Adjusted to ntr={args.ntr}, nte={args.nte}')
    ...
    print(f'Training samples: {len(u_tr)}, Test samples: {len(u_te)}')
    print('\n')
