...
class FNO(torch.nn.Module):
    def __init__(self, modes, vis_channels, hidden_channels, proj_channels, x_dim=1, t_scaling=1000):
        ...
        # neuralop >= 0.3 uses projection_channel_ratio instead of projection_channels
        proj_ratio = proj_channels / hidden_channels if hidden_channels > 0 else 2
        self.model = _FNO(n_modes=n_modes, 
                         hidden_channels=hidden_channels, 
                         projection_channel_ratio=proj_ratio,
                         in_channels=in_channels, out_channels=vis_channels,
                         positional_embedding=None)  # We handle positional embedding ourselves
